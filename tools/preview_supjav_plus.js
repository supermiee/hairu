/*
 * SupJav+ 预览工具：node tools/preview_supjav_plus.js
 * 在 Node 桩里跑 supjav_core + supjav_pages，把 setResult/setHomeResult 的卡片
 * 渲成近似海阔布局的 HTML（docs/dev/preview_supjav_plus.html），浏览器打开即可目检 UI。
 */
'use strict';
var fs = require('fs');
...
var out = path.join(ROOT, 'docs', 'dev', 'preview_supjav_plus.html');
fs.mkdirSync(path.dirname(out), { recursive: true });
fs.writeFileSync(out, html);
console.log('preview:', homeCards ? homeCards.length + ' home cards, ' + detail.length + ' detail cards' : 'EMPTY');
console.log('open https://supermiee.github.io/hairu/dev/preview_supjav_plus.html after push');
