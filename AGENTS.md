...
- **Diagnostics added for that (core 1.3.0 / pages v4)**: detail-page rows `🩺 播放自检` (route `renderDiagnostics` → `core.diagnose(id)`, on-device: geo / cdn-access / master / per-variant playlist time+size+segment count / ⑤ first-byte latency / ⑥⑦ two 3 MB `Range` downloads for a sustained KB/s reading / ⑧ a bandwidth verdict / ⑨⑩ what is handed to the player and the container type), `📋 复制直连播放地址` (`remotePlayUrl(id,'720P')`), `🌐 用原站播放器播放`, and a `🧪 固定清晰度自测` row (`playQuality` → `core.qualityUrl(id,q)`, plays one pinned bitrate with no ABR).
