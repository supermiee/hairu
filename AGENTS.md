...
- **Known unresolved issue (2026-09): AV01 playback stutters in Hiker's native player while the site plays fine at the same resolution.** Ruled out with the user: bitrate/ABR (v1.1.0 didn't help), playlist size (a normal 1–4 h video's playlist is only ~170–500 KB and still stutters; our 3 MB tokenized playlist only happens for 20 h movies), decode mode (hard/soft makes no difference), and the CDN path (identical hosts/tokens to the browser). What's left: the stream is **CMAF fMP4** (`.m4s` + `#EXT-X-MAP`, segments 0.7–4.4 MB) with a **354-char `access_token` query on every segment URL**, while the apps that play smoothly (Jable/MissAV/SupJav) all use **MPEG-TS segments with a clean URL**. Hiker bundles **ExoPlayer 2.15.1** (`VideoPlayModule` + `-Lite`, a GSY fork with `MyHlsExtractorFactory`/`MyHlsPlaylistTracker` and `setAllowChunklessPreparation(true)`); its actual player entry (`PlayerChooser`) is missing from the reference clone, so this can't be reproduced here. Server-side TS alternatives were probed and are dead: `master-ts.m3u8` hangs/400, `…/file90-N-….ts` on the CDN 504s, `/v2/<storage>/video.mp4` 404s (only `preview.mp4`, a trailer, exists).
- **v1.5.0 / pages v6**: quality UI is driven by the video's real variants — the `🧪 固定清晰度自测` row lists only the qualities that actually exist (a sampled 45 videos: 42 had 360/720/1080, one had only 720P, one 360/720P, so some do lack 1080P), and `resolveMedia` falls back to an uncapped master if the 720P cap would empty it (e.g. a 1080P-only video) so we never hand the player an empty playlist.
- **v1.4.0 / pages v5 mitigation**: the local ABR master is now built with `CONFIG.limits.abrMaxHeight` (720P) — `buildMaster(text,id,token,maxHeight)` drops variant + I-FRAME entries above the cap exactly like the site's own `Lce()` does. 1080P stays as a manual line and as a `🧪 固定清晰度自测` button. Rationale: the phone's link measures ~400–560 KB/s of actual transfer (415 KB in 2.5 s minus the 1.77 s first-byte latency) but its per-request latency is 1.5–1.8 s, so letting ABR probe 1080P (needs 392 KB/s sustained) makes it thrash.
- **Diagnostics added for that (core 1.4.0 / pages v5)**: detail-page rows `🩺 播放自检` (route `renderDiagnostics` → `core.diagnose(id)`, on-device: geo / cdn-access / master / per-variant playlist time+size+segment count / ⑤ first-byte latency / ⑥⑦ two 3 MB `Range` downloads for a sustained KB/s reading / ⑧ a bandwidth verdict / ⑨⑩ what is handed to the player and the container type), `📋 复制直连播放地址` (`remotePlayUrl(id,'720P')`), `🌐 用原站播放器播放`, and a `🧪 固定清晰度自测` row (`playQuality` → `core.qualityUrl(id,q)`, plays one pinned bitrate with no ABR).
- **First device reading (2026-09, video 192318)**: every request from the phone took **1.2–1.8 s** (high latency) and a 500 KB range read came back at **~149 KB/s ≈ 1.2 Mbps**. AV01's declared bitrates are 360P 0.39 Mbps / 720P 1.38 / 1080P 3.14 ⇒ at ~1.2 Mbps only **360P** is sustainable, 720P is borderline, 1080P is impossible. So the site being smooth at the "same resolution" is very likely hls.js's ABR having silently dropped to 360P. **MX Player cannot play the stream at all** (FFmpeg HLS `.m4s`/`#EXT-X-MAP`) and Hiker's X5 webview can't run hls.js either (no MSE) — so neither is usable as a comparison/fallback. The decisive test is the new pinned-quality button: **if pinned 360P is smooth → bandwidth/ABR; if 360P also stutters → the fMP4/player path.**
- State keys are prefixed `av01.`. Preview: `node tools/preview_av01.js` → `docs/dev/preview_av01.html`. Fixtures: `test/fixtures/av01_{home,latest,hottest,detail,similars,search,actresses,makers,tags,actress_videos,geo}.json` + `av01_master.m3u8` + `av01_variant.m3u8` (real API responses; descriptions/translations trimmed; `av01_variant.m3u8` is a real tokenized variant playlist truncated to 6 segments).
