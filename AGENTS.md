...
- **Diagnostics added for that (core 1.2.0 / pages v3)**: detail-page rows `🩺 播放自检` (route `renderDiagnostics` → `core.diagnose(id)`, run on-device: geo / cdn-access / master / per-variant playlist time+size+segment count / a `Range` segment speed test / container type), `📋 复制直连播放地址` (`remotePlayUrl(id,'720P')` → paste into VLC/MX to separate player vs stream), and `🌐 用原站播放器播放` (`video://<page>#isVideo=true#`, the site's own hls.js in the webview). Run these before changing anything else.
