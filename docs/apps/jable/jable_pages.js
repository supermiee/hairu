...
(function () {
    ...
    function listRouteBySource(pageSource, params) {
        return $('hiker://empty#' + pageSource).rule(function (payload) {
            ...
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1').renderList({ url: source, title: payload.title, listKind: payload.listKind, selectedSort: payload.selectedSort, searchKeyword: payload.searchKeyword, searchSort: payload.searchSort }); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js').renderList({ url: source, title: payload.title, listKind: payload.listKind, selectedSort: payload.selectedSort, searchKeyword: payload.searchKeyword, searchSort: payload.searchSort }); }
        }, params);
    }
    ...
    function routeModels(url, title) {
        return $('hiker://empty#' + pagedSource(url)).rule(function (payload) {
            var source = String(MY_URL || '').split('#')[1] || payload.url;
            source = source.split('@rule=')[0];
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1').renderModels({ url: source, title: payload.title }); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js').renderModels({ url: source, title: payload.title }); }
        }, { url: url, title: title || '女優' });
    }
    function routeDetail(item) {
        return $('hiker://empty').rule(function (payload) {
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1').renderDetail(payload); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js').renderDetail(payload); }
        }, { url: item.url, title: item.title || '', image: item.image || '' });
    }
    function routePage(name, title) {
        return $('hiker://empty').rule(function (payload) {
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1')[payload.name](); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js')[payload.name](); }
        }, { name: name, title: title || '' });
    }
    function routeVerification() {
        return $('hiker://empty').rule(function () {
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1').renderVerification(); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js').renderVerification(); }
        }, {});
    }
    function routeTaxonomyPage() {
        return $('hiker://empty').rule(function () {
            try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_pages.js?v=1').renderTaxonomy(); }
            catch (ignore) { $.require('hiker://files/rules/jable/jable_pages.js').renderTaxonomy(); }
        }, {});
    }
    ...
    function emptyRuleToggle(item) {
        return $('hiker://empty').lazyRule(function (payload) {
            var app;
            try { app = requirejs('https://supermiee.github.io/hairu/apps/jable/jable_core.js?v=1'); }
            catch (ignore) { app = $.require('hiker://files/rules/jable/jable_core.js'); }
    ...
    function renderSettings() {
        ...
        setResult([
            ...
            { title: '清除緩存與本地數據', url: $('hiker://empty').lazyRule(function () {
                try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_core.js?v=1').clearLocal(); } catch (ignore) { $.require('hiker://files/rules/jable/jable_core.js').clearLocal(); }
                return 'toast://已清除';
            }), col_type: 'text_center_1' }
        ]);
    }
        ...
        for (var i = 0; i < app.config.languages.length; i++) {
            ...
            result.push({
                ...
                url: $('hiker://empty').lazyRule(function (languageId) {
                    var app;
                    try { app = requirejs('https://supermiee.github.io/hairu/apps/jable/jable_core.js?v=1'); } catch (ignore) { app = $.require('hiker://files/rules/jable/jable_core.js'); }
    ...
    function renderVerification() {
        ...
        setResult([
            ...
            { title: '第二步：驗證成功後，點此返回並刷新', url: $('hiker://empty').lazyRule(function () {
                ...
                try { requirejs('https://supermiee.github.io/hairu/apps/jable/jable_core.js?v=1').clearPageCache(); } catch (ignore) { $.require('hiker://files/rules/jable/jable_core.js').clearPageCache(); }
                back(true);
